# === ЧАСТЬ 1: Импорт, настройки, константы ===
...
# === ЧАСТЬ 2: Баланс, лимиты, индикаторы ===
def get_balance():
    try:
...
# === ЧАСТЬ 5: Торговая логика, отчёт, запуск ===
CYCLE_COUNT = 0
LAST_REPORT_DATE = None

def trade():
    global CYCLE_COUNT
    usdt = get_balance(); log(f"💰 USDT Баланс: {usdt:.2f}")
    ...
    for sym in SYMBOLS:
        try:
            ...
            phase = detect_market_phase(df); price = df["c"].iloc[-1]
            state = STATE[sym]; now=time.time(); params=DEFAULT_PARAMS
            ...
            for pos in state["positions"]:
                ...
                elif draw>=params["max_drawdown_sl"]:
                    session.place_order(category="spot", symbol=sym, side="Sell", orderType="Market", qty=str(q))
                    log_trade(sym,"STOP LOSS",price,q,pnl,f"🔻 SL: {draw:.2%}")
                    state["pnl"]+=pnl; state["avg_count"]=0
                else:
                    pos["tp"]=calc_adaptive_tp(price,atr,q,params)
                    pos["peak_price"]=peak; new_pos.append(pos)
            state["positions"]=new_pos

            if sig=="buy":
                if phase=="bear" or (phase=="sideways" and df["rsi"].iloc[-1]<=45):
                    continue
                if not state["positions"]:
                    qty=calc_adaptive_qty(usdt,atr,price,sym,params["risk_pct"]); amt=qty*price
                    if qty and amt>=LIMITS[sym]["min_amt"] and amt<=MAX_POSITION_SIZE_USDT:
                        tp=calc_adaptive_tp(price,atr,qty,params); profit=(tp-price)*qty-price*qty*0.001
                    ...
                    b0=state["positions"][0]["buy_price"]
                    draw=(b0-price)/b0
                    if phase in ["bull","sideways"] and draw>=params["min_avg_drawdown"] and now-state["last_avg_time"]>=COOLDOWN_AVG_SECS and df["rsi"].iloc[-1]>50:
                        qty=calc_adaptive_qty(usdt/2,atr,price,sym,params["risk_pct"]); amt=qty*price
