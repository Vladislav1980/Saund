...
# - netPnL >= $1.5 после комиссий
# - ДОПОЛНИТЕЛЬНО (включается флагами):
#   * USE_VOLUME_FILTER: проверка, что объём свечи не «тонкий»
#   * USE_ORDERBOOK_GUARD: проверка спреда и ожидаемого VWAP-импакта заявки
# Оба фильтра можно отключать, логика TP/SL/ProfitOnly не меняется.
...
OB_LIMIT_DEPTH      = 25
MAX_SPREAD_BP       = 15     # 0.15% макс спред
MAX_IMPACT_BP       = 20     # 0.20% макс ожидаемый импакт VWAP
...
def volume_ok(df) -> bool:
    ...
    if last_vol < VOL_FACTOR_MIN * vol_ma:
        logging.info(f"⏸ Volume guard: last_vol<{VOL_FACTOR_MIN}*MA")
        return False
...
def trade_cycle():
    ...
    if now.hour == DA ILY_REPORT_HOUR and now.minute >= DAILY_REPORT_MINUTE and LAST_REPORT_DATE != now.date():
        # (опечатка «DA ILY» исключи у себя, здесь просто напомнил: DAILY_REPORT_HOUR)
        pass
