...
def init_state():
    ...
    log("✅ Loaded full state from Redis" if STATE else "ℹ Starting fresh, no previous state in Redis", True)
...
def init_positions():
    ...
    for sym in SYMBOLS:
        ...
        log(f"DEBUG: balance for {sym} at start = {bal:.6f}", True)
        df = get_kline(sym)
    ...
    if not STATE["TONUSDT"]["positions"]:
        restore_price, restore_qty = 3.596, 5.28
        restore_tp = restore_price + (MIN_NET_PROFIT / restore_qty)
        STATE["TONUSDT"]["positions"].append({
            "buy_price": restore_price, "qty": restore_qty, "tp": restore_tp,
            "timestamp": datetime.datetime.now().isoformat()
        })
        msgs.append(f"- TONUSDT restored manually: {restore_qty} @ {restore_price:.4f} (${restore_qty*restore_price:.2f})")
        total += restore_qty * restore_price
        log("🧩 RESTORED TONUSDT manually into Redis", True)
    save_state()
    header = "🚀 Bot started"
    header += "\n💰 Активные позиции:\n" + "\n".join(msgs) + f"\n📊 Итого: ${total:.2f}" if msgs else "\n💰 Активных позиций нет"
    log(header, True)
...
def trade():
    ...
    for sym in SYMBOLS:
        ...
        df = get_kline(sym)
        if df.empty:
            log(f"{sym} Kline empty — skipped", True)
            continue
        sig, atr, info = signal(df)
        price = df["c"].iloc[-1]; coin_bal = get_coin_balance(sym); value = coin_bal * price
        logging.info(f"[{sym}] signal={sig}, price={price:.4f}, value={value:.2f}, pos_count={len(st['positions'])}, indicators={info}")
        ...
        if sig == "buy":
            ...
            if last_stop and hrs < 4:
                log(f"{sym}: Skipped BUY — only {hrs:.1f}h since last stop-loss", True)
                continue
            if st["positions"] and st["avg_count"] < MAX_AVERAGES:
                # logic for averaging — same checks, we log reason if skipping
                log(f"{sym}: Potential averaging skipped — avg_count {st['avg_count']} or value>=per_sym or price_diff too small", True)
            elif not st["positions"] and value < per_sym:
                # initial buy path (with calculations of drawdown etc.)
                # we log reason if skipping
                log(f"{sym}: Initial BUY considered", True)
            else:
                log(f"{sym}: BUY skipped — existing position or not enough free USDT", True)

    cycle_count += 1
    now = datetime.datetime.now()
    if now.hour == 22 and now.minute >= 30 and LAST_REPORT_DATE != now.date():
        send_tg("📊 Daily report:\n" + "\n".join(f"{s}: PnL={STATE[s]['pnl']:.2f}" for s in SYMBOLS))
