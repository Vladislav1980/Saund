    ...
    for sym in SYMBOLS:
        ...
        if pos:
            ...
            if cb > 0 and sym in LIMITS:
                ...
                if qty == 0:
                    log(f"{sym} на балансе есть, но после округления qty=0")
                    continue

                sell_signal = last5["ema9"] < last5["ema21"] and last5["macd"] < last5["macd_s"]
                last_buy_price = state.get("last_manual_buy", 0)
                threshold_pct = 0.05
                threshold_atr = DEFAULT_PARAMS["tp_multiplier"] * atr
                threshold_price_pct = last_buy_price * (1 + threshold_pct) if last_buy_price else 0
                threshold_price_atr = last_buy_price + threshold_atr if last_buy_price else 0
                profit_sell = last_buy_price > 0 and price >= max(threshold_price_pct, threshold_price_atr)
...
def main():
    ...
    send_tg("🚀 Bot старт: основной 5m, фильтр 15m/1h/4h")
    while True:
        trade()
        daily_report()
        time.sleep(60)
