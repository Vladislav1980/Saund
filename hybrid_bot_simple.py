...
# - netPnL >= $1.5 после комиссий (ProfitOnly)
...
# Порог «продать только в прибыль»
MIN_PROFIT_PCT  = 0.005     # 0.5% от ноторнала (доп. фильтр)
MIN_ABS_PNL     = 0.0
MIN_NET_PROFIT  = 1.50      # не ниже $1.5
MIN_NET_ABS_USD = 1.50      # минимум чистыми (после комиссий) $1.5
...
def orderbook_ok(sym: str, side: str, qty_base: float, ref_price: float) -> bool:
    ...
    try:
        ...
        if not asks or not bids:
            logging.info("⏸ OB guard: empty orderbook")
            return False
        ...
        # на сетевых ошибках не блокируем
        return True
...
def trade_cycle():
    ...
    for sym in SYMBOLS:
        try:
            ...
            # объёмный фильтр (только информативный лог здесь);
            # при покупках он учитывается ещё раз «жёстко»
            if not volume_ok(df):
    ...
    # Daily report (исправленный блок без опечатки)
