...
def signal(df, sym):
    ...
    ratio = get_orderbook_ratio(sym)
    if ratio < DEFAULT_PARAMS["min_bid_ask_ratio"]: return "none", last["atr"]
...
def trade():
    ...
    for sym in SYMBOLS:
        ...
        if pos:
            ...
            held = now - pos["time"]
            peak = max(peak, price)
            if price >= tp or df["rsi"].iloc[-1] >= 80 or price <= peak*(1 - DEFAULT_PARAMS["trailing_stop_pct"]) or dd >= DEFAULT_PARAMS["max_drawdown_sl"] or (sig == "sell" and pnl > 0) or held > 12*3600:
                session.place_order(category="spot", symbol=sym, side="Sell", orderType="Market", qty=str(q))
                reason = ("TP" if price >= tp else "RSI" if df["rsi"].iloc[-1] >= 80 else "Trail" if price <= peak*(1 - DEFAULT_PARAMS["trailing_stop_pct"]) else "SL" if dd >= DEFAULT_PARAMS["max_drawdown_sl"] else "Bear" if sig=="sell" else "Held")
            ...
            if sig == "buy":
                ...
                if q * price * 0.001 >= DEFAULT_PARAMS["min_profit_usd"] * 1.5:
                    session.place_order(category="spot", symbol=sym, side="Sell", orderType="Market", qty=str(q))
                    pnl = q * price * 0.999
                    log(f"⚠️ SELL BALANCE {sym}@{price:.4f}, qty={q:.6f}")
                    send_tg(f"⚠️ SELL BALANCE {sym}@{price:.4f}, qty={q:.6f}")
        if state["pos"]:
...
def daily_report():
    ...
    if d.hour == 22:
        ...
        send_tg(rep)
        for s in SYMBOLS: STATE[s]["count"], STATE[s]["pnl"] = 0, 0.0
        save_state()
