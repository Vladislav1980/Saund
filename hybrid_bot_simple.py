...
DEFAULT_PARAMS = {"risk_pct":.03, "tp_multiplier":1.8, "trailing_stop_pct":.02,
                  "max_drawdown_sl":.06, "min_profit_usdt":2.5, "volume_filter":.3,
                  "avg_rebuy_drop_pct":.07, "rebuy_cooldown_secs":3600}
...
if "last_report_day" not in STATE: STATE["last_report_day"] = ""
...
def trade():
    ...
    for sym in SYMBOLS:
        ...
        ok_count = 0
        for tf in ["15","60","240"]:
            df = get_klines(sym, tf,100)
            if df.empty:
                mts[tf] = "no data"; continue
            df = signal(df); ok, last = check_trend(df)
            if ok: ok_count += 1
            mts[tf] = f"{'OK' if ok else 'fail'} EMA9={last['ema9']:.2f},21={last['ema21']:.2f},MACD={last['macd']:.4f}/{last['macd_s']:.4f},RSI={last['rsi']:.1f},ATR={last['atr']:.4f}"

        if sig in ["buy","sell"]:
            if ok_count < 2:
                log(f"{sym} 5m={sig.upper()}, но MTF не подтвердил:")
                for tf,st in mts.items(): log(f"  {tf}m -> {st}")
                sig = "none"

        price, atr, state = last5["c"], last5["atr"], STATE[sym]
        pos = state["pos"]
        ...
        # BUY logic
            ...
            qty = adjust(qty_usd/price, LIMITS.get(sym,{}).get("step",1))
            if qty <= 0: log(f"{sym} qty=0 после округления buy"); continue
            if qty*price < LIMITS[sym]["min_amt"]:
                log(f"{sym} пропущен: {qty*price:.2f} < min_amt"); continue
            est = atr*DEFAULT_PARAMS["tp_multiplier"]*qty - price*qty*0.001 - DEFAULT_PARAMS["min_profit_usdt"]
            if est<0:
                log(f"{sym} пропущен: est_profit={(est+DEFAULT_PARAMS['min_profit_usdt']):.2f} < min"); continue
        ...
        # SELL by signal or profit with balance
        elif sig=="sell" or get_coin_balance(sym)>0:
            cb = get_coin_balance(sym)
            qty = adjust(cb, LIMITS.get(sym,{}).get("step",1))
            if qty <= 0: log(f"{sym} qty=0 после округления sell"); continue
            if sig=="sell":
                session.place_order(category="spot",symbol=sym,side="Sell",orderType="Market",qty=str(qty))
                txt=f"SELL SIGNAL {sym}@{price:.4f}, qty={qty:.6f}"
                log(txt); send_tg(txt); state["pos"]=None
            elif "last_manual_buy" in state:
                lb = state["last_manual_buy"]
                if price >= lb*(1+0.05) or price >= lb + DEFAULT_PARAMS["tp_multiplier"]*atr:
                    session.place_order(category="spot",symbol=sym,side="Sell",orderType="Market",qty=str(qty))
                    txt=f"SELL PROFIT {sym}@{price:.4f}, qty={qty:.6f}, от {lb:.4f}"
                    log(txt); send_tg(txt); state["pos"]=None
                else:
                    log(f"{sym}: баланс {cb:.4f}, но условия для sell не выполнены")
        ...
        if state["pos"]:
            ...
            if price<=buy*(1-DEFAULT_PARAMS["avg_rebuy_drop_pct"]) and now-state["last_rebuy"]>=DEFAULT_PARAMS["rebuy_cooldown_secs"]:
                ...
                if extra_qty*price >= LIMITS[sym]["min_amt"]:
                    ...
                    avg_p = (buy*state["pos"]["qty"] + price*extra_qty)/total_qty
                    state["pos"].update({"buy_price":avg_p,"qty":total_qty,"tp":avg_p+DEFAULT_PARAMS["tp_multiplier"]*atr})
                    state["last_rebuy"]=now
                    txt=f"AVERAGE {sym}: +{extra_qty:.6f}@{price:.4f} => avg={avg_p:.4f}"
                    log(txt); send_tg(txt)
...
def daily_report():
    ...
    if d.hour == 22 and STATE.get("last_report_day","") != today:
        rep = "📊 Ежедневный отчет\n" + "\n".join(f"{s}: trades={STATE[s]['count']}, pnl={STATE[s]['pnl']:.2f}" for s in SYMBOLS)
...
def main():
    log("🚀 Bot start full logs + MTF (5m primary, фильтр 15/60/240m)")
    send_tg("🚀 Bot старт. Основной TF 5m, фильтр MTF 15/60/240m")
    while True:
        trade()
        daily_report()
        time.sleep(60)
