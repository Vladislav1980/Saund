# -*- coding: utf-8 -*-
"""
Bybit Spot Bot — v3 + NetPnL‑трейлинг
    ...
    for sym in SYMBOLS:
        try:
            ...
            if price and bal * price >= lm.get("min_amt", 0.0) and bal >= lm.get("min_qty", 0.0):
                ...
                STATE[sym]["positions"] = [{"buy_price": price, "qty": q_net,
                                            "buy_qty_gross": q_net/(1-TAKER_FEE), "tp": tp, "max_pnl": 0.0}]
...
if __name__ == "__main__":
    logging.info("🚀 Bot starting with NetPnL‑trailing logic")
    tg_event("🚀 Bot starting with NetPnL‑trailing logic")
    init_state()
    load_symbol_limits()
    restore_positions()
    tg_event(
