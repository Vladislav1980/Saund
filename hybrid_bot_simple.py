# -*- coding: utf-8 -*-
"""
Bybit Spot Bot — v3 + NetPnL-трейлинг + Profit tiers + Absolute Avg threshold
Сохраняет оригинальный функционал, добавляет:
- фиксацию netPnL >= 1.5 USDT без ожидания возврата к 1.5 (трейлинг от пика),
- быстрый выход на уровнях прибыли 5/10/20/50 при развороте,
- усреднение только при ощутимом откате (по USD или %).
"""
...
# ATR-TP лишь как ориентир (TP поле в позиции), основная фиксация — через netPnL
TRAIL_MULTIPLIER = 1.5

# Разрешённая глубина усреднения (старая логика по %)
MAX_DRAWDOWN = 0.10
MAX_AVERAGES = 3

# Stop-Loss (используется только если позиция уже достигала минимума прибыли)
STOP_LOSS_PCT = 0.03

# Требование к минимальному профиту (net, после комиссий)
MIN_PROFIT_PCT = 0.005
MIN_ABS_PNL = 0.0
MIN_NET_PROFIT = 1.50
MIN_NET_ABS_USD = 1.50

# Трейлинг по прибыли (net PnL)
TRAIL_PNL_TRIGGER = 1.5   # от этой прибыли включаем слежение за пиком
TRAIL_PNL_GAP = 0.6       # фиксация, если от пика откатилось на 0.6

# Уровни "крупной" прибыли — реакция быстрее (при развороте)
PROFIT_TIERS = [1.5, 5.0, 10.0, 20.0, 50.0]

# Порог усреднения в абсолютных USD и процентах
AVG_MIN_DRAWDOWN_USD = 5.0
AVG_MIN_DRAWDOWN_PCT = 0.015  # 1.5%
...
def volume_ok(df: pd.DataFrame):
    ...
    if last_vol < VOL_FACTOR_MIN * vol_ma:
        ...
        return False, f"vol_guard"
    if notional < MIN_CANDLE_NOTIONAL:
        logging.info(f"⏸ Volume guard: notional {notional:.2f} < {MIN_CANDLE_NOTIONAL:.2f} USDT")
        return False, f"vol_guard_notional"
    return True, "vol=ok"
...
def orderbook_ok(sym: str, side: str, qty_base: float, ref_price: float):
    ...
    try:
        ...
        if spread > MAX_SPREAD_BP/10000.0:
            return False, f"ob=spread {spread*100:.2f}%"
        if side.lower() == "buy":
            ...
            if impact > MAX_IMPACT_BP/10000.0:
                return False, f"ob=impact {impact*100:.2f}%"
...
def can_place_buy(sym: str, q: float, price: float, usdt_free: float) -> (bool, str):
    ...
    lm = LIMITS[sym]
    if q < lm["min_qty"] or q * price < lm["min_amt"]:
        return False, "below_limits"
    need = q * price * (1 + TAKER_FEE + SLIP_BUFFER)
    ok = need <= max(0.0, usdt_free - RESERVE_BALANCE + 1e-9)
    return ok, (f"need={need:.2f}" if ok else f"need>{max(0.0, usdt_free-RESERVE_BALANCE):.2f}")
...
def daily_report():
    try:
        ...
        for sym in SYMBOLS:
            ...
            lines.append(
                f"{sym}: balance={bal:.6f} (~{val:.2f} USDT), trades={s['count']}, "
                f"pnl={s['pnl']:.2f}, maxDD={s['max_drawdown']*100:.2f}%, curPosQty={cur_q:.6f}"
            )
        tg_event("\n".join(lines))
    except Exception as e:
    ...
    for sym in SYMBOLS:
        try:
            ...
            for p in state["positions"]:
                ...
                # Чистая прибыль по продаваемой части
                ...
                # 1) Быстрый выход на крупных уровнях при признаках разворота
                if any(pnl >= tier for tier in PROFIT_TIERS[1:]) and is_bearish_reversal(df) and can_place_sell(sym, sell_cap_q, price, coin_bal):
                    _attempt_sell(sym, sell_cap_q)
                    msg = f"✅ TIER SELL {sym} @ {price:.6f}, qty={sell_cap_q:.8f}, netPnL={pnl:.2f}"
                    logging.info(msg); tg_event(msg)
                    STATE[sym]["pnl"] += pnl; STATE[sym]["last_sell_price"] = price; STATE[sym]["avg_count"] = 0
                    left = q_n - sell_cap_q
                    if left > 0:
                ...
                # 2) Трейлинг от пика: как только пик >= 1.5 и откат >= 0.6 — продаём
                if p["max_pnl"] >= TRAIL_PNL_TRIGGER and (p["max_pnl"] - pnl) >= TRAIL_PNL_GAP and can_place_sell(sym, sell_cap_q, price, coin_bal):
                    _attempt_sell(sym, sell_cap_q)
                ...
                # 3) Обычная фиксация прибыли (как только pnl >= need)
                if pnl >= need and can_place_sell(sym, sell_cap_q, price, coin_bal):
                    _attempt_sell(sym, sell_cap_q)
                ...
                # 4) SL — только если уже ок к фиксации (избегаем фиксации убытка по шуму)
                if price <= b * (1 - STOP_LOSS_PCT) and pnl >= need and can_place_sell(sym, sell_cap_q, price, coin_bal):
                    _attempt_sell(sym, sell_cap_q)
                ...
                # иначе держим, подтягиваем вспомогательный tp
            ...
            if sig == "buy" and vol_ok:
                # если уже есть позиция — усредняем только при ощутимом откате
                if state["positions"] and state["avg_count"] < (MAX_AVERAGES - 1):
                    ...
                    dd_pct = (price - avg_price) / max(avg_price, 1e-12)  # < 0 — ниже среднего
                    dd_usd = (avg_price - price) * total_q if price < avg_price else 0.0

                    allow_avg = (dd_pct < 0) and ((abs(dd_pct) >= AVG_MIN_DRAWDOWN_PCT) or (dd_usd >= AVG_MIN_DRAWDOWN_USD))
                    if allow_avg:
                        ...
                        if q_gross > 0 and ob_ok and can_buy:
                            ...
                            if _attempt_buy(sym, q_gross):
                                ...
                                append_pos(sym, price, q_gross, tp_new)
                                after = json.dumps(state["positions"], indent=2, ensure_ascii=False)
                                state["count"] += 1
                                state["avg_count"] += 1
                                qty_net = q_gross * (1 - TAKER_FEE)
                                msg = f"🟢 BUY(avg) {sym} @ {price:.6f}, qty_net={qty_net:.8f} | dd_pct={dd_pct:.4f}, dd_usd={dd_usd:.2f}, {ob_info}, {vol_info}"
                                logging.info(msg); tg_event(msg)
                                tg_event(f"📊 AVG {sym} POSITION UPDATE\nДо:\n{before}\nПосле:\n{after}")
                                coins = get_wallet(True); usdt = usdt_balance(coins); avail = max(0.0, usdt - RESERVE_BALANCE)
                        else:
                            logging.info(f"[{sym}] ❌ Skip avg: not allowed (dd_pct={dd_pct:.4f}, dd_usd={dd_usd:.2f}); {vol_info}")
                    else:
                        logging.info(f"[{sym}] 🔸Skip avg: dd too small (pct={dd_pct:.4f}, usd={dd_usd:.2f})")
                # новой позиции нет — открываем обычную покупку
                    ...
                    if state["last_sell_price"] and abs(price - state["last_sell_price"]) / price < 0.003:
                        ...
                        if q_gross > 0 and ob_ok and can_buy:
                            ...
                            logging.info(f"[{sym}] ❌ Skip buy: {ob_info}, can_buy={can_buy}({why_buy}), free={usdt:.2f}, avail={avail:.2f}, {vol_info}")
                else:
...
if __name__ == "__main__":
    logging.info("🚀 Bot starting with NetPnL-trailing + Profit tiers + Absolute Averaging threshold")
    tg_event("🚀 Bot starting with NetPnL-trailing + Profit tiers + Absolute Averaging threshold")
    init_state()
    load_symbol_limits()
    restore_positions()
    tg_event(
        ...
        f"AvgUSD={AVG_MIN_DRAWDOWN_USD}, AvgPct={AVG_MIN_DRAWDOWN_PCT*100:.2f}%, "
        f"NetPnL min={MIN_NET_ABS_USD}, Trail(trigger={TRAIL_PNL_TRIGGER}, gap={TRAIL_PNL_GAP}), "
        f"Tiers={PROFIT_TIERS}"
    )
    while True:
        try:
            trade_cycle()
